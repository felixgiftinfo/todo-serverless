...
namespace Todo_Serverless.Functions
{
    public class GetCompletedTodos
    {
        ...
        public GetCompletedTodos(ITodoAPIService todoAPIService)
        {
        ...
        public async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Function,  "get", Route = "getCompletedTodos")] HttpRequest req)
        {
            var auth = _todoAPIService.AccessTokenProvider.ValidateToken(req);
            if (auth.Status == AccessTokenStatus.Valid)
            {
                var cmd = new GetCompletedTodoQuery(auth.DatabaseName);
