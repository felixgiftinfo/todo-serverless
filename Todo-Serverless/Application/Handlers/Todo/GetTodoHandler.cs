...
namespace Todo_Serverless.Application.Handlers
{
    ...
    public class GetTodoHandler : IRequestHandler<GetTodoQuery, IEnumerable<TodoReadDTO>>
    {
        private readonly ILogger _logger;
        private readonly ITodoAPIService _service;
        public GetTodoHandler(ITodoAPIService todoAPIService, ILogger<GetTodoHandler> log)
        {
            _service = todoAPIService;
            _logger = log;
        }

        public async Task<IEnumerable<TodoReadDTO>> Handle(GetTodoQuery request, CancellationToken cancellationToken)
        {
            _logger.LogInformation("Call to GetTodo function made");
            ...
            using (var session = await this._service.DatabaseEngine.Client.StartSessionAsync())
            {
                ...
                //var todo = this._service.DatabaseEngine.GetTable<Object>(request.DatabaseName, TableName.TODO_TABLE_NAME);
                var todo = this._service.DatabaseEngine.GetTable<TodoModel>(request.DatabaseName, TableName.TODO_TABLE_NAME);
                var results = await todo.Find(session, x => true).ToListAsync();
