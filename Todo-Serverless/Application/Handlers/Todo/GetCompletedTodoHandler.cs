...
namespace Todo_Serverless.Application.Handlers
{
    ...
    public class GetCompletedTodoHandler : IRequestHandler<GetCompletedTodoQuery, IEnumerable<TodoReadDTO>>
    {
        private readonly ILogger _logger;
        private readonly ITodoAPIService _service;
        public GetCompletedTodoHandler(ITodoAPIService todoAPIService, ILogger<GetCompletedTodoHandler> log)
        {
            _service = todoAPIService;
            _logger = log;
        }

        public async Task<IEnumerable<TodoReadDTO>> Handle(GetCompletedTodoQuery request, CancellationToken cancellationToken)
        {
            _logger.LogInformation("Call to GetCompletedTodo function made");
            ...
            using (var session = await this._service.DatabaseEngine.Client.StartSessionAsync())
            {
                ...
                var results = await todo.Find(session, x => x.Completed == true).ToListAsync();
                ...
                _logger.LogInformation("Call to GetCompletedTodo function completed.");
